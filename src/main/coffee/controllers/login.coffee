...
angular.module('beeSolarApp')
  .controller 'LoginCtrl', ($scope, Auth, $location, $http, Base64) ->
    ...
    $scope.doAuth =  ->
        ...
        $scope.wasRejected = false
        # SaltedAuth
        saltedPass = Base64.encode($scope.pasword + ":" + $scope.sToken)
        $scope.isSending = true
        $http.post('/auth', {token: $scope.sToken, username: $scope.username, password: saltedPass}).success((sData, iStatus, oHeaders) ->
            $scope.isSending = false
            if(sData == "OK")
                $scope.isReady = true
